...
## [2026-08-29] ingest | Demystifying evals for AI agents (Anthropic Engineering)
- ソース: `raw/articles/demystifying-evals-for-ai-agents.md`(sha256:0dc62c80793c、2026-01-09 公開、Mikaela Grace / Jeremy Hadfield / Rodrigo Olivares / Jiri De Jonghe)。台帳未登録のため新規 ingest。原本は記事 HTML の `<article>` 要素を markdown 化して一字一句保存(見出し・表・YAML コード例・リンクを保持。図は Next.js の画像 URL のまま)。
- 追加したページ: [[agent-evaluation]](ハブ。語彙定義・なぜ難しいか・なぜ作るか・エージェント種別4分類・手法の重ね合わせ・フレームワーク一覧), [[graders]](3分類と設計原則、eval 自体のバグ), [[capability-vs-regression-evals]](2種の eval と飽和), [[pass-at-k]](pass@k / pass^k), [[eval-driven-development]](Step 0〜8 のロードマップ)
- 更新したページ: [[evaluators]] — 「Anthropic の grader 分類との対応」節を新設し、Strands の組み込み evaluator が LLM judge に偏っていることと「経路の採点」への留保を注記。[[experiment-management]] — Case/Experiment を task/suite という手法論側の語彙に接続。[[strands-agents-evals]] — 「位置づけ」節を新設し evaluation harness 層の1選択肢として Harbor/Braintrust/LangSmith/Langfuse/Arize と並置。[[spec-driven-development]] — TDD 系譜の節に eval-driven development との同型性と相違(二値 vs 成功率)を追記。[[agentic-engineering]] — 「Quality calibration」の実務化として本記事群へリンク。
- 主な学び: (1) **outcome と transcript を分ける**——「予約が完了しました」という発話ではなく環境の SQL に予約行があるかを見る。エージェント評価の核心。(2) 「エージェントを評価する」とは **agent harness(scaffold)とモデルの組**を評価すること。低スコアの原因はモデル・harness・task 仕様・grader のどこにでもありうる。(3) grader 設計の中心原則は「**経路ではなく成果物を採点する**」——ツール呼び出し順の固定はエージェントの妥当な創造性を罰し、テストを脆くする。(4) **pass@k(k↑で上昇)と pass^k(k↑で下降)** は k=1 で一致し k=10 で正反対の物語を語る。顧客向けエージェントは pass^k。(5) **eval 自体がバグる**——Opus 4.5 は CORE-Bench で 42% だったが、硬直した数値比較・曖昧な task 仕様・再現不能な確率的 task を修正して 95% に。METR の time horizon ベンチマークでも「閾値まで最適化せよ」と指示しつつ採点は超過を要求する誤設定が見つかり、指示に従うモデルが罰せられていた。(6) 経験則「**frontier モデルで 0% pass@100 は、能力不足でなく task が壊れている signal**」。(7) 着手のハードルは低い——**実際の失敗から取った 20〜50 task で十分**。待つほど作りにくくなる(稼働中システムから成功基準をリバースエンジニアリングする羽目になる)。(8) **両方向のバランス**が要る。Claude.ai のウェブ検索 eval では「検索すべきクエリ」と「既存知識で答えるべきクエリ」の両方を作らないと、何にでも検索するエージェントができあがる。(9) trial の隔離は性能を**下げる**方向だけでなく**上げる**方向にも効く——社内 eval で Claude が前の trial の git 履歴を覗いて不当な優位を得ていた。(10) 運用モデルとして「**専任 evals チームが中核インフラを持ち、eval task の大半はドメイン専門家とプロダクトチームが貢献する**」が最も効果的だった。PM やカスタマーサクセスが Claude Code で eval task を PR として出せる。
- 矛盾・要確認: (a) 既存 [[evaluators]](Strands ドキュメント)は「複数 evaluator を組み合わせる」を前面に出すが、本記事は「**決定論的を可能な限り優先し、LLM judge は必要なときだけ**」と既定値の置き所が異なる。矛盾ではないが力点の相違として [[evaluators]] に併記した。(b) 同じく Strands の `TrajectoryEvaluator` の `exact_match_scorer` / `in_order_match_scorer` は「ツール呼び出し順序の採点」であり、本記事の「経路でなく成果物を採点せよ」と正面から緊張する。順序一致が本当に要件かを確認して使うべき旨を注記。(c) 記事は Opus 4.5 の 𝜏2-bench 事例で「eval 上は失敗だがユーザーにとってはより良い解」を挙げており、**eval スコア = 良さ ではない**ことを記事自身が認めている。これは記事の主要主張(eval に投資せよ)を弱めるものではないが、スコアの一人歩きへの歯止めとして [[agent-evaluation]] に ⚠️ で明示した。(d) 事例(Descript / Bolt / Qodo / Claude Code)はいずれも Anthropic 側の記述による定性的な紹介で、**測定データや対照は示されていない**。「eval に投資したチームは開発が加速する」という中心主張に定量的裏付けはなく、実務家の経験則として読むべき。
