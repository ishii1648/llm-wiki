...
[Section titled “Main Idea of Multi-agent System”](#main-idea-of-multi-agent-system)
...
[Section titled “High Level Commonality in Graph, Swarm and Workflow”](#high-level-commonality-in-graph-swarm-and-workflow)
...
[Section titled “Difference in Graph, Swarm and Workflow”](#difference-in-graph-swarm-and-workflow)
...
| Execution Flow | Controlled but Dynamic.  The flow follows graph edges, but an LLM’s decision at each node determines the path. | Sequential & Autonomous.  An agent performs a task and then hands off control to the most suitable peer. | Deterministic & Parallel.  The flow is fixed by the dependency graph. Independent tasks run in parallel. |
...
| Conversation History | Full Transcript. The entire dialogue history is part of the shared state, giving every agent complete and open context. | Shared Transcript. The shared context provides a full history of agent handoffs and knowledge contributed by previous agents, available to the current agent. | Task-Specific context.  A task receives a curated summary of relevant results from its dependencies, not the full history. |
...
### When to Use [Graph](/docs/user-guide/concepts/multi-agent/graph/)

[Section titled “When to Use Graph”](#when-to-use-graph)
...
### When to Use [Swarm](/docs/user-guide/concepts/multi-agent/swarm/)

[Section titled “When to Use Swarm”](#when-to-use-swarm)
...
- Software Development: As shown in the [`Swarm` documentation](/docs/user-guide/concepts/multi-agent/swarm/#how-swarms-work), a researcher hands off to an architect, who hands off to a coder, who hands off to a reviewer. The path is emergent.

### When to Use [Workflow](/docs/user-guide/concepts/multi-agent/workflow/)

[Section titled “When to Use Workflow”](#when-to-use-workflow)
...
[Section titled “Shared State Across Multi-Agent Patterns”](#shared-state-across-multi-agent-patterns)

- [Python](#tab-panel-1743)
...
[Section titled “How Shared State Works”](#how-shared-state-works)

The `invocation_state` is automatically propagated to:

- All agents in the pattern via their `**kwargs`
- Tools via `ToolContext` when using `@tool(context=True)` - see [Python Tools](/docs/user-guide/concepts/tools/custom-tools/#accessing-state-in-tools)
- Tool-related hooks (BeforeToolCallEvent, AfterToolCallEvent) - see [Hooks](/docs/user-guide/concepts/agents/hooks/#accessing-invocation-state-in-hooks)
...
[Section titled “Accessing Shared State in Tools”](#accessing-shared-state-in-tools)
...
[Section titled “How Invocation State Works”](#how-invocation-state-works)
...
[Section titled “Accessing Invocation State in Tools”](#accessing-invocation-state-in-tools)
...
[Section titled “MultiAgentState”](#multiagentstate)
...
[Section titled “Important Distinctions”](#important-distinctions)
...
- [Graph Documentation](/docs/user-guide/concepts/multi-agent/graph/)
- [Swarm Documentation](/docs/user-guide/concepts/multi-agent/swarm/)
- [Workflow Documentation](/docs/user-guide/concepts/multi-agent/workflow/)
